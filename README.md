...
With the services running, open a new terminal and navigate to the `best-buy-makeline-service` directory.
...
To run the necessary services, clone the repo, open a terminal, and navigate to the `best-buy-makeline-service` directory. Then run the following command:
...
Using the [`test-best-buy-makeline-service.http`](./test-best-buy-makeline-service.http) file in the root of the repo, you can test the API. However, you will need to use VS Code and have the [REST Client](https://marketplace.visualstudio.com/items?itemName=humao.rest-client) extension installed.
